...
static BiNode nodeThr;
static BiTree headThr = &nodeThr;
static BiTree preThr = NULL;

int sDoThreading(BiTree T)
{
    if (!T)
        return 0;
    sDoThreading(T->left);
    ...
    if (preThr->right == NULL) {
        preThr->right = T;
        preThr->rtag = THREADING;
    }
    preThr = T;
    sDoThreading(T->right);
    return 0;
}
int sDoThrMain(BiTree T)
{
    memset(headThr, 0, sizeof(BiNode));
    headThr->left = T;
    headThr->ltag = THREADING;

    preThr = headThr;
    sDoThreading(T);
    preThr->right = headThr;
    preThr->rtag = THREADING;

    printf("This is threading: ");
    BiTree n = headThr->right;
    while (n != headThr) {
        printf("%2d ", n->data);
        n = n->right;
    }
